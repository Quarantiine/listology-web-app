...
const FilterBar = () => {
	...
	return (
		<div className="filter-bar flex flex-col justify-center items-start gap-5 w-full">
			<div
				className={`${themeMode[0]?.mode ? "text-white" : "text-black"} border w-full h-16 rounded-lg ${
					themeMode[0]?.mode ? "bg-[#222] border-[#444]" : "bg-[#e9e9e9] border-[#b8b8b8]"
				...
				<div className="w-full h-fit p-1 flex justify-between items-center gap-5 px-5 sm:px-10">
					{folders.length > 0 && folderClicked && (
						...
						>
							{addTodo}
							{todoLists.length < 1 && (
								<div className="absolute -top-[56px] -right-3 base-bg w-fit h-fit p-2 text-white animate-bounce rounded-full">
									<p className="text-sm">Add Todo</p>
								</div>
							)}
						</button>
					)}
...
const FilterIcons = ({ themeMode, layoutView }) => {
	...
	const heart = (
		<svg width="17" height="23" viewBox="0 0 24 23" fill="none" xmlns="http://www.w3.org/2000/svg">
			<path
				...
				fill={themeMode[0]?.mode ? `#222` : `white`}
				stroke={themeMode[0]?.mode ? `white` : `black`}
				strokeWidth="2"
			/>
		</svg>
